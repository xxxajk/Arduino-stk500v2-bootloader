...
// define this if GCC makes progmem backwards. It does this for me, very annoying!
#define COMPILER_BACKWARDS_PROGMEM
...
#elif defined( _BOARD_MEGA16_ )
/*****************************************************************************
 * on board led is PORTA7
 *****************************************************************************/
#define                       PROGLED_PORT PORTA
#define                        PROGLED_DDR DDRA
...
/*****************************************************************************
 * no on board LED but this is what will probably be added to this port
 *****************************************************************************/
#define                       PROGLED_PORT PORTB
#define                        PROGLED_DDR DDRB
...
#define                      APP_ARENA_END (FLASHEND - BOOTSIZE + 1)
#define                 TRAMPOLINE_OFFSET (APP_ARENA_END + _VECTORS_SIZE)
...
#if (FLASHEND < 0x10000)
#define RAMPOFFSET(x) ((address_t)((uint16_t)&x))
#define PRAMPOFFSET(x) ((address_t)((uint16_t)x))
#else
#define RAMPOFFSET(x) ((address_t)((FLASHEND & 0xFFFF0000LU) | (uint16_t)&(x)))
#define PRAMPOFFSET(x) ((address_t)((FLASHEND & 0xFFFF0000LU) | (uint16_t)(x)))
#endif
...
typedef struct pinfo {
        ...
        int16_t size;
        uint32_t address;
} __attribute__((packed)) pinfo_t;

//extern void program_block(pinfo_t *d);

__attribute__((__used__)) void program_block_(pinfo_t *d) {
        address_t address = d->address;
        address_t eraseAddress = address & ~((uint32_t) (SPM_PAGESIZE - 1));
        uint8_t *p = d->data;
        int16_t size = d->size;
...
void __jumpMain(void) {
        // July 17, 2010   <MLS> Added stack pointer initialization
...
int main(void) {
                        ...
                        switch(msgBuffer[0]) {
                                ...
                                case CMD_PROGRAM_EEPROM_ISP:
                                {
                                        ...
                                        if(msgBuffer[0] == CMD_PROGRAM_FLASH_ISP) {
                                                eraseAddress = address & ~((uint32_t) (SPM_PAGESIZE - 1));
                                                // erase only main section (bootloader protection)
                                                if(eraseAddress < APP_END) {
                                                        boot_page_erase(eraseAddress); // Perform page erase
                                                        boot_spm_busy_wait(); // Wait until the memory is erased.
        ...
        {
                // Force a reference to program_block_ in order to produce a trampoline.
                // This generates some bloated code...
                volatile address_t _program_block_address __attribute__((__unused__)) = RAMPOFFSET(program_block_);
        }
...
#if !defined(COMPILER_BACKWARDS_PROGMEM)
...
const char gTextMsg_Explorer[] PROGMEM = "stk500V2 bootloader by MLS, fixed by AJK";
...
const char gTextMsg_WriteToEEprom[] PROGMEM = "Writing EE";
...
const char gTextMsg_FlashWrite[] PROGMEM = "FlashWrite  = ";
...
void PrintFromPROGMEM(address_t dataPtr, unsigned char offset) {
        ...
        do {
#if (FLASHEND > 0x10000)
                theChar = pgm_read_byte_far((address_t) dataPtr++);
#else
                theChar = pgm_read_byte_near((address_t) dataPtr++);
#endif
                if(theChar != 0) {
                        sendchar(theChar);
                }
...
static void PrintCPUstats(void) {
        ...
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_Explorer), 0);

        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_COMPILED_ON), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_GCC_DATE_STR), 0);

        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_CPU_Type), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_CPU_Name), 0);

        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_AVR_ARCH), 0);
        PrintDecInt(__AVR_ARCH__, 1);
        PrintNewLine();

        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_GCC_VERSION), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_GCC_VERSION_STR), 0);

        // these can be found in avr/version.h
        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_AVR_LIBC), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_AVR_LIBC_VER_STR), 0);

#if defined(SIGNATURE_0)
        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_CPU_SIGNATURE), 0);
        ...
        // fuse settings
        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_FUSE_BYTE_LOW), 0);
        fuseByte = boot_lock_fuse_bits_get(GET_LOW_FUSE_BITS);
        PrintHexByte(fuseByte);
        PrintNewLine();

        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_FUSE_BYTE_HIGH), 0);
        fuseByte = boot_lock_fuse_bits_get(GET_HIGH_FUSE_BITS);
        PrintHexByte(fuseByte);
        PrintNewLine();

        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_FUSE_BYTE_EXT), 0);
        fuseByte = boot_lock_fuse_bits_get(GET_EXTENDED_FUSE_BITS);
        PrintHexByte(fuseByte);
        PrintNewLine();

        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_FUSE_BYTE_LOCK), 0);
        fuseByte = boot_lock_fuse_bits_get(GET_LOCK_BITS);
        PrintHexByte(fuseByte);
        PrintNewLine();
        // _VECTORS_SIZE contains the offset to the trampoline
        // TRAMPOLINE_OFFSET should == program_block_ trampoline
        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_FlashWrite), 0);
        fuseByte = (PRAMPOFFSET(TRAMPOLINE_OFFSET) >> 16)&0xff;
        PrintHexByte(fuseByte);
        fuseByte = (PRAMPOFFSET(TRAMPOLINE_OFFSET) >> 8)&0xff;
        PrintHexByte(fuseByte);
        fuseByte = (PRAMPOFFSET(TRAMPOLINE_OFFSET))&0xff;
        PrintHexByte(fuseByte);
        PrintNewLine();
...
#define  kDUMP_FLASH 0
#define kDUMP_EEPROM 1
#define    kDUMP_RAM 2

/************************************************************************/

static void DumpHex(register uint8_t dumpWhat, register uint8_t numRows, unsigned long startAddress) {
        //unsigned long myAddressPointer;
        //uint16_t ii;
        uint8_t theValue;
        uint8_t asciiDump[32];
        uint8_t *ramPtr;
        ...
        while(numRows > 0) {
                if(startAddress > 0x10000) {
                        PrintHexByte((startAddress >> 16) & 0x00ff);
                }
                PrintHexByte((startAddress >> 8) & 0x00ff);
                PrintHexByte(startAddress & 0x00ff);
                sendchar(0x20);
                sendchar('-');
                sendchar(0x20);
                ...
                for(int ii = 0; ii < 16; ii++) {
                        switch(dumpWhat) {
                                case kDUMP_FLASH:
#if (FLASHEND > 0x10000)
                                        theValue = pgm_read_byte_far((address_t) startAddress);
#else
                                        theValue = pgm_read_byte_near((uint16_t) startAddress);
#endif
                                        break;

                                case kDUMP_EEPROM:
                                        theValue = eeprom_read_byte((uint8_t *) ((uint16_t) startAddress));
                                        break;

                                case kDUMP_RAM:
                                        theValue = ramPtr[(uint16_t) startAddress];
                                        break;
...
static void EEPROMtest(void) {
        ...
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_WriteToEEprom), 0);
        PrintNewLine();
        ii = 0;
#if (FLASHEND > 0x10000)
        while(((theChar = pgm_read_byte_far(RAMPOFFSET(gTextMsg_Explorer) + ii)) != '*') && (ii < 512))
#else
        while(((theChar = pgm_read_byte_near(RAMPOFFSET(gTextMsg_Explorer) + ii)) != '*') && (ii < 512))
#endif
        {
                eeprom_write_byte((uint8_t *) ii, theChar);
                if(theChar == 0) {
                        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_SPACE), 0);
                } else {
                        sendchar(theChar);
                }
                ii++;
        ...
        PrintNewLine();
        PrintNewLine();
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_ReadingEEprom), 0);
        PrintNewLine();
        errorCount = 0;
        ii = 0;
#if (FLASHEND > 0x10000)
        ...
        PrintNewLine();
        PrintNewLine();
        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_EEPROMerrorCnt), 0);
...
static void VectorDisplay(void) {
        ...
        vectorIndex = 0;
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_CPU_Name), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_VECTOR_HEADER), 0);
        // V#   ADDR   op code
        ...
        while(vectorIndex < kInterruptVectorCount) {
                ...
                if(word1 == 0xffff) {
                        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_noVector), 0);
                        ...
                        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_rjmp), 0);
                        ...
                        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_jmp), 0);
...
static void PrintAvailablePort(char thePortLetter) {
        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_PORT), 0);
...
static void AVR_PortOutput(void) {
        ...
        PrintFromPROGMEM(RAMPOFFSET(gTextMsg_WHAT_PORT), 0);
        ...
        if((portLetter >= 'A') && (portLetter <= 'Z')) {
                ...
                switch(portLetter) {
                        ...
                        default:
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_PortNotSupported), 0);
                                getCharFlag = false;
                                break;
                }
                if(getCharFlag) {
                ...
                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_MustBeLetter), 0);
        }
}
...
static void PrintHelp(void) {
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_0), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_QM), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_AT), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_B), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_E), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_F), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_H), 0);

        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_L), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_Q), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_R), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_V), 0);
        PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_Y), 0);
}
...
static void RunMonitor(void) {
        ...
        keepGoing = 1;
        while(keepGoing) {
                PrintFromPROGMEM(RAMPOFFSET(gTextMsg_Prompt), 0);
                theChar = recchar();
                if(theChar >= 0x60) {
                ...
                switch(theChar) {
                        case '0':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_0), 2);
                                gFlashIndex = 0;
                                gRamIndex = 0;
                                gEepromIndex = 0;
                                break;
                        ...
                        case '?':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_QM), 2);
                                PrintCPUstats();
                                break;

                        case '@':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_AT), 2);
                                EEPROMtest();
                                break;

                        case 'B':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_B), 2);
                                BlinkLED();
                                break;

                        case 'E':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_E), 2);
                                DumpHex(kDUMP_EEPROM, 16, gEepromIndex);
                                gEepromIndex += 256;
                        ...
                        case 'F':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_F), 2);
                                DumpHex(kDUMP_FLASH, 16, gFlashIndex);
                                gFlashIndex += 256;
                                break;

                        case 'H':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_H), 2);
                                PrintHelp();
                                break;

                        case 'L':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_L), 2);
                                ListAvailablePorts();
                                break;

                        case 'Q':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_Q), 2);
                                keepGoing = false;
                                break;

                        case 'R':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_R), 2);
                                DumpHex(kDUMP_RAM, 16, gRamIndex);
                                gRamIndex += 256;
                                break;

                        case 'V':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_V), 2);
                                VectorDisplay();
                                break;

                        case 'Y':
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_HELP_MSG_Y), 2);
                                AVR_PortOutput();
                                break;

                        default:
                                PrintFromPROGMEMln(RAMPOFFSET(gTextMsg_SPACE), 0);
                                break;
                }
        }
}
...
#if defined(COMPILER_BACKWARDS_PROGMEM)
#if 0
// Don't enable, causes a double trampoline.
__asm__(
        "       .global __AJKMagic__\n"
